...
router.get("/available/teacher/:teacherID/day/:day/period/:periodNo/nperiods/:nperiods", async (req, res) => {
  ...
  try {
    ...
    const newPeriodEnd = periodNo + nperiods - 1; // Adjusted to correctly calculate the end period
    const newPeriodStart = periodNo;
    for (let c of allClasses) {
      const classEnd = c.startingPeriod + c.noOfPeriod - 1; // Adjusted to correctly calculate the end period
      const classStart = c.startingPeriod;

      // Check for overlap
      if (newPeriodStart <= classEnd && newPeriodEnd >= classStart) {
        availability = {
          available: false,
          overlapClass: c,
          overlapAt: classStart <= newPeriodStart ? classStart : newPeriodStart
        };
        break;
      }
    }
